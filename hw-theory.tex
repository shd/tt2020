...
\item Докажите, что если $a,b,k \in \mathbb{N}$ и $a + b < k$, то $a < k$ и $b < k$.
...
\section*{Домашнее задание №11: <<Линейная логика и уникальные типы>>}

\begin{enumerate}
\item Как известно, интуиционистские связки могут быть выражены в линейном исчислении.
Покажите соответствующие интуиционистские аксиомы для следующих выражений интуиционистских связок
через линейные:
\begin{enumerate}
...
\item Уникальные типы. Рассмотрим интерфейс массива в линейных типах:

$$
\begin{array}{rl}
new: & !Val \multimap (Arr \multimap Arr \otimes X) \multimap X\\
lookup: & !Ix \multimap Arr \multimap Arr\ \otimes \ !Val\\
update: & !Ix \multimap \ !Val \multimap Arr \multimap Arr\\
\end{array}
$$

Определите соответствующий интерфейс в уникальных типах.

\item Типизируйте следующие конструкции в системе с уникальными типами:
\begin{enumerate}
\item $\lambda a.\lambda b.b\ a$
\item $\lambda a.a\ a$
\item $\lambda a.\lambda b.a$
\item $\lambda a.\lambda b.\lambda c.a\ c\ (b\ c)$
\end{enumerate}

\end{enumerate}

\section*{Домашнее задание №12: <<Формализация подтипов>>}
...
$$\infer{\Gamma\vdash\forall X <: U_1.S_2 <: \forall X <: U_1.T_2}{\Gamma,X<:U_1\vdash S_2 <: T_2}$$
\end{tabular}\end{center}

\begin{enumerate}
\item Покажите, что $X <: B, Y <: X \vdash B\rightarrow Y <: X \rightarrow B$ 

\item Определим $\neg S ::= \forall X <: S. X$. Покажите, что
$\Gamma\vdash\neg S <: \neg T$ тогда и только тогда, когда
$\Gamma\vdash S <: T$.
...
\section*{Задания на баллы}
Каждое задание будет оценено в 10 баллов, сдавать можно любые, в любой момент. Решением является
программа на языке Аренд.

\begin{enumerate}
\item Рассмотрим алгорифм Эвклида, использующий деление: алгоритм, по натуральным числам $a$ и $b$
возвращающий их наибольший общий делитель. Для точности укажем шаг этого алгоритма:
$$(a_{n+1},b_{n+1}) := \left\{\begin{array}{ll}(b_n, a_n \textrm{ mod } b_n) & \text{если } a_n \ge b_n > 0\\
                                               (b_n, a_n) &\text{если } a_n < b_n\end{array}\right.$$
Формализуйте этот алгорифм и покажите, что он:
\begin{enumerate}
\item всегда заканчивается;
\item итоговая пара чисел выглядит как $(\textrm{НОД}(a,b), 0)$
\end{enumerate}

\item Рассмотрим кольцо вычетов по модулю $p$:

\begin{verbatim}
TBD
\end{verbatim}

Определите в Аренде утверждение <<существует делитель нуля>> ($\exists a.\exists b.a \ne 0 \with b \ne 0 \with a \cdot b = 0$) и
покажите, что если $p$ --- простое, то делителей нуля в \verb!QuotientRing p! нет.

\item Рассмотрим список \verb!Data.List! и определим на нём операцию обмена: 
$$swap(v,a,b)[n] = \left\{\begin{array}{ll}
         a, &\textrm{если } n = b\\
         b, &\textrm{если } n = a\\
         v[n], &\textrm{иначе}\end{array}\right.$$

Покажите, что $\forall v.\forall a.\forall b.swap(v,a,b) = swap(v,b,a)$
\end{enumerate}
