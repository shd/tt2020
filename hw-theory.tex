...
Если мы доказываем утверждение, следующее из выражения под квантором существования,
и нам надо воспользоваться соответствующим примером или доказательством, мы можем на них сослаться.
Ниже $p$ --- это доказательство $\exists x^\texttt{Nat}.x^2=77$, и элементы пары индексируются 
натуральными числами; $p.1$ --- это пример значения, а $p.2$ --- доказательство утверждения
$(p.1)^2 = 77$:
